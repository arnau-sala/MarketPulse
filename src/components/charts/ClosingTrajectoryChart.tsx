...
// Distribute totalUplift across future weeks with a ramp starting at Week 3
function upliftAtStep(step: number, totalSteps: number, totalUplift: number): number {
  const activateFrom = Math.min(2, totalSteps - 1);
      ...
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3 mb-5">
        <div>
          ...
          <p className="text-[12px] text-ink-500 mt-0.5">
            La <span className="text-[#D97706] font-semibold">línea naranja</span> es la previsión sin acciones.
            La <span className="text-[#15803D] font-semibold">línea verde</span> muestra el resultado
            ejecutando el plan recomendado (activación Off-Trade en Semana 3).
          </p>
        </div>
          ...
          <ResponsiveContainer width="100%" height={300}>
            <ComposedChart data={data} margin={{ top: 8, right: 16, left: 0, bottom: 0 }}>
              ...
              <Legend
                ...
                iconType="circle"
                iconSize={8}
                formatter={(value) => (value === 'Area base' || value === 'Zona de ganancia') ? null : value}
              />
            </ComposedChart>
          </ResponsiveContainer>
          ...
          <div className="flex items-center justify-between mt-3 pt-3 border-t border-ink-100">
            ...
            <div className="flex items-center gap-4 text-[11px]">
              <span className="text-ink-400">
                Sin acciones:{' '}
                <span className={noActionGap >= 0 ? 'text-success font-semibold' : 'text-danger font-semibold'}>
                  {noActionGap >= 0 ? '+' : ''}{fmtShort(noActionGap)}
                </span>
              </span>
              {potentialGain > 0 && (
                <span className="text-ink-400">
                  Ganancia con el plan:{' '}
