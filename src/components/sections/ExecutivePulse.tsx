  ...
  const handleGenerateBriefing = async () => {
    ...
    try {
      const res = await generateBriefing({
        ...
        status: m.status,
        topGapDriver: 'Bajo rendimiento Off-Trade',
        recommendedAction: 'Activar plan de Recuperación Equilibrada en Semana 3',
      });
      setBriefing(res.text);
      setBriefingModel(res.model);
    } catch {
      setBriefing('El servicio de briefing no está disponible. Asegúrate de que el backend está en marcha y GROQ_API_KEY está configurada.');
    } finally {
    ...
    <div className="space-y-6">
      ...
      {loadingF ? (
        <div className="bg-white rounded-2xl border border-ink-300/60 shadow-card px-5 py-5 h-[340px] flex items-center justify-center text-ink-400 text-[13px]">
          <Loader2 size={16} className="animate-spin mr-2" /> Cargando previsión…
        </div>
      ) : (
        ...
        <div className="bg-white px-5 py-5">
          {briefing ? (
            ...
            <div className="flex flex-col items-center justify-center py-8 gap-3 text-ink-400">
              ...
              <p className="text-[11px] text-ink-400">El modelo procesa ventas, previsiones y planes de acción.</p>
            </div>
          ) : (
            <div className="flex flex-col items-center justify-center py-8 gap-3 text-center">
              ...
              <p className="text-[12px] text-ink-400 max-w-sm">
                Pulsa <strong>Generar</strong> para obtener un resumen ejecutivo basado en los datos reales de este mes.
              </p>
            </div>
          )}
        </div>
          ...
          <div className="grid grid-cols-3 gap-3">
            <div className="flex items-start gap-2">
              ...
              <div>
                ...
                <p className="text-[10px] text-ink-400 leading-snug">175 semanas de ventas reales UK (2022–2026)</p>
              </div>
            </div>
            <div className="flex items-start gap-2">
              ...
              <div>
                ...
                <p className="text-[10px] text-ink-400 leading-snug">+32% calibrado en 18 semanas promo 2026 × Off-Trade 1.08×</p>
              </div>
            </div>
            <div className="flex items-start gap-2">
              ...
              <div>
                ...
                <p className="text-[10px] text-ink-400 leading-snug">Semana 3 = 1.32× demanda (payday UK) → £66k incremental</p>
              </div>
            </div>
          </div>
        </div>
