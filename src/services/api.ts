...
import type { PromoIntensity, Channel, Brand, Week } from '../types';
...
export async function getForecast(horizon = 13): Promise<ForecastPoint[]> {
...
export async function getChannelPerformance(): Promise<ChannelPerformance[]> {
  return get<ChannelPerformance[]>('/api/channel-performance');
}

export async function getBrandPerformance(): Promise<BrandPerformance[]> {
  return get<BrandPerformance[]>('/api/brand-performance');
}
